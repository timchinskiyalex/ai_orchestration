...
test("only all required candidate-SHA CI contexts permit merge", async () => {
  const scenarios = [
    { name: "green unrelated", runs: [{ name: "lint", status: "completed", conclusion: "success", head_sha: "a".repeat(40) }], expected: "blocked_ci" },
    { name: "missing", runs: [], expected: "blocked_ci" },
    { name: "pending", runs: [{ name: "build", status: "in_progress", conclusion: null, head_sha: "a".repeat(40) }], expected: "blocked_ci" },
    { name: "failed", runs: [{ name: "build", status: "completed", conclusion: "failure", head_sha: "a".repeat(40) }], expected: "blocked_ci" },
    { name: "green required", runs: [{ name: "build", status: "completed", conclusion: "success", head_sha: "a".repeat(40) }], expected: "completed_merged" }
  ];
    ...
    try {
      const github = {
        ...
        async api(args) {
          ...
          if (path.includes("check-runs")) return { stdout: JSON.stringify({ check_runs: scenario.runs }) };
          if (path.endsWith("/status")) return { stdout: JSON.stringify({ sha: integration.manifest.candidateSha, statuses: [] }) };
          throw new Error(`unexpected API ${path}`);
        }
      };
      const result = await router.publishCandidate(integration, {
        remoteGitAdapter: { async pushCandidate({ sha }) { return { status: "pushed", verifiedSha: sha }; } },
        pullRequestAdapter: { async ensurePullRequest({ sha }) { return { status: "open", number: 50, url: "https://example.test/pr/50", headSha: sha }; } },
        remoteCiAdapter: new GitHubCiAdapter({ github, requiredContexts: ["build"], timeoutMs: 1, pollIntervalMs: 1 }),
        mergeAdapter: { async merge() { merged += 1; return { status: "merged", mainSha: "b".repeat(40), targetVerified: true }; } }
      });
      assert.equal(result.terminalState, scenario.expected, scenario.name);
      assert.equal(merged, scenario.expected === "completed_merged" ? 1 : 0, scenario.name);
    } finally { router.close(); rmSync(root, { recursive: true, force: true }); }
  }
});
...
test("retry after a merge side effect detects the already-merged PR without repeating merge", async () => {
  ...
  try {
    const adapters = {
      remoteGitAdapter: { async pushCandidate({ sha }) { return { status: "pushed", verifiedSha: sha }; } },
      pullRequestAdapter: { async ensurePullRequest({ sha }) { return { status: "open", number: 77, url: "https://example.test/pr/77", headSha: sha, duplicate: mergeCalls > 0 }; } },
      remoteCiAdapter: { async waitForChecks() { return { status: "passed" }; } },
      mergeAdapter: { async merge() { mergeCalls += 1; if (mergeCalls === 1) { remoteMerges += 1; throw new RemoteAdapterError("credentials", "connection dropped after merge"); } return { status: "merged", mainSha: "b".repeat(40), targetVerified: true, duplicate: true }; } }
    };
    const first = await router.publishCandidate(integration, adapters);
    assert.equal(first.terminalState, "blocked_credentials"); assert.equal(remoteMerges, 1);
    const second = await router.publishCandidate(integration, adapters);
    assert.equal(second.terminalState, "completed_merged"); assert.equal(remoteMerges, 1); assert.equal(mergeCalls, 2);
  } finally { router.close(); rmSync(root, { recursive: true, force: true }); }
});
