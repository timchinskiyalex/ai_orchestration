...
test("generated App Server schema contains the protocol methods used by the client", { skip: !schemaPath ? "set CODEX_APP_SERVER_SCHEMA to a generated Codex schema" : false }, () => {
  ...
  for (const method of ["initialize", "initialized", "thread/start", "turn/start", "turn/completed", "account/read", "account/usage/read", "account/rateLimits/read", "account/rateLimits/updated"]) assert.equal(schema.includes(method), true, `missing ${method}`);
