...
test("npm run integrate dispatches to the integration CLI", () => {
  ...
  try {
    const config = { project: { name: "test" }, repository: root, runtimeDir: join(root, "runtime"), baseRef: "main", roles: Object.fromEntries(["bootstrap", "planner", "backend", "frontend", "database", "qa", "security", "devops"].map((role) => [role, { sandbox: "read-only", approvalPolicy: "never", tokenBudget: 100, maxAttempts: 1, usesWorktree: false }])) };
