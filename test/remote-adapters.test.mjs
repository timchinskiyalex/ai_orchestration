...
test("required CI contexts are evaluated only for the exact candidate SHA", async () => {
  ...
  const missing = await verify({ runs: [check("build", "completed", "success")] }); assert.equal(missing.status, "timed_out"); assert.equal(missing.required[1].state, "missing");
  const pending = await verify({ runs: [check("build", "in_progress", null), check("lint", "completed", "success")] }); assert.equal(pending.status, "timed_out"); assert.equal(pending.required[0].state, "pending");
  const failed = await verify({ runs: [check("build", "completed", "failure"), check("lint", "completed", "success")] }); assert.equal(failed.status, "failed");
  const stale = await verify({ runs: [check("build", "completed", "success", "b".repeat(40)), check("lint", "completed", "success")] }); assert.equal(stale.status, "timed_out"); assert.equal(stale.required[0].state, "missing");
  const passed = await verify({ runs: [check("build", "completed", "success"), check("lint", "completed", "success")] }); assert.equal(passed.status, "passed"); assert.deepEqual(passed.requiredContexts, ["build", "lint"]);
});
...
test("GitHub merge refuses a PR whose head mutated after green candidate CI", async () => {
  ...
  const github = { async repositoryName() { return "owner/repo"; }, async api(args) { const path = args.find((item) => typeof item === "string" && item.startsWith("repos/")); if (path === "repos/owner/repo/pulls/9") return { stdout: JSON.stringify({ number: 9, head: { sha: "b".repeat(40) }, base: { ref: "main" } }) }; if (path.includes("/merge")) { mergeCalls += 1; return { stdout: "{}" }; } throw new Error(`Unexpected GitHub API request: ${path}`); } };
