...
class FakeAppServer extends EventEmitter {
  ...
  async readThread({ threadId }) { const thread = this.threads.get(threadId); const text = /^Plan /.test(thread.goal) ? "```json\n{\"tasks\":[{\"id\":\"fix-value\",\"title\":\"Fix value\",\"prompt\":\"Fix value\",\"primaryDomain\":\"backend\",\"supportingDomains\":[\"security\",\"qa\"],\"riskFlags\":[\"auth_or_authorization\"],\"humanApprovalRequired\":false,\"estimatedTokens\":20,\"dependsOn\":[],\"allowedPaths\":[\"src/value.mjs\"],\"acceptanceChecks\":[\"npm test\"]}]}\n```" : /^Bootstrap /.test(thread.goal) ? "```json\n{\"summary\":\"ok\",\"assumptions\":[],\"risks\":[],\"humanGates\":[]}\n```" : "verification complete"; return { thread: { turns: [{ id: thread.turnId, items: [{ type: "agentMessage", text }] } ] } }; }
}
test("quota-free full flow materializes security/QA gates, finalizes writer, and integrates", async () => {
  ...
  try {
    ...
    router = new SwarmRouter({ repository: root, runtimeDir: join(root, "runtime"), baseRef: "main", model: "fake", project: { name: "fake", documentationDir: "docs/orchestration-input", generatedDir: "docs/orchestration-generated" }, router: { maxConcurrentTasks: 1, maxChildrenPerTask: 5, maxDelegationDepth: 4, maxPlanTasks: 5, defaultParentBudget: 1000, turnTimeoutMs: 1000, approvalMode: "deny" }, budget: { weeklyTokenLimit: 10000, weeklyWindowDays: 7 }, quota: { throttleAtUsedPercent: 90, throttleWhenUnavailable: false }, roles, appServerClientFactory: () => new FakeAppServer() });
    await router.ensureProjectOverlay(); const bootstrap = router.startProject(); await router.runUntilIdle(); const planner = router.approveHumanGate(bootstrap.id).next; await router.runUntilIdle();
    ...
    assert.ok(backend && security && qa, JSON.stringify(tasks.map((task) => ({ role: task.role, status: task.status, error: task.error })))); assert.equal(backend.status, "awaiting_human"); assert.deepEqual(security.dependencies, [backend.id]); assert.deepEqual(qa.dependencies, [security.id]);
    router.approveHumanGate(planner.id); router.approveHumanGate(backend.id); await router.runUntilIdle();
    assert.equal(router.store.getTask(backend.id).status, "done"); const artifact = router.store.workerArtifactRecord(backend.id); assert.ok(artifact?.path);
    const integration = await router.integrateFinalized([backend.id]); assert.equal(integration.manifest.status, "awaiting_human_merge");
