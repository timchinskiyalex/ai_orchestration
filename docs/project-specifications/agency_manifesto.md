...
Output:
# Agency Manifesto вЂ” European Trip Guide Site
...
booking tips. The guides are the product вЂ” they must be genuinely useful, not
...
**TECH_SPEC.md** at the project root вЂ” every agent should read it, not just
...
- Never invent facts you're not confident about (exact opening hours, exact
  prices) вЂ” write "check current price/hours before booking" instead of a
  made-up number.
...
- Locked (unpurchased) place content is a real access-control requirement,
  not just a UI nicety вЂ” it must be enforced server-side.
- Shell commands (run_command) must never require interactive input вЂ” always
