...
export class Integrator {
  ...
  async #integrateUnlocked({ artifacts, overlay }) {
    ...
    try {
      ...
      return this.#writeManifest({ id, status: "awaiting_human_merge", branch, worktree, baseSha: overlay.repository.baseSha, candidateSha: headSha, headSha, appliedArtifacts: applied, verificationResults, localVerification: { status: "passed", commands: verificationResults }, remoteCi: this.#extensionStatus("remote CI", this.integration.remoteCiExtension), pullRequest: this.#extensionStatus("pull request", this.integration.pullRequestExtension), blockedReason: null, rollback: { baseSha: overlay.repository.baseSha, resetCommand: `git switch ${branch} && git reset --hard ${overlay.repository.baseSha}` }, recovery: { mode: "preserved-worktree", worktree, action: "Review the clean candidate branch, then remove it with git worktree remove after the explicit merge decision." }, humanMergeGate: { required: true, targetBranch: overlay.repository.branch, candidateSha: headSha, action: "explicit SHA-bound human merge or PR" } });
