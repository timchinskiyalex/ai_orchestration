...
export class StateStore {
  ...
  resumeDeliveryRun(id, { ownerPid, ownerSessionId }) {
    ...
    try {
      ...
      const timestamp = now(); const changed = this.db.prepare("UPDATE delivery_runs SET state = 'running', owner_pid = ?, owner_session_id = ?, heartbeat_at = ?, interrupted_at = NULL, updated_at = ? WHERE id = ? AND owner_session_id IS NULL").run(ownerPid, ownerSessionId, timestamp, timestamp, id);
  ...
  resumeInterruptedTasks(deliveryRunId) {
    ...
    if (!tasks.length) return [];
    this.db.exec("BEGIN IMMEDIATE");
    try { const timestamp = now(); for (const task of tasks) { this.db.prepare("UPDATE tasks SET status = 'queued', error = NULL, updated_at = ? WHERE id = ?").run(timestamp, task.id); this.#insertEvent(task.id, "task/resumed", { from: "interrupted", to: "queued", recovery: "same_delivery_run" }); } this.db.exec("COMMIT"); }
    catch (error) { this.db.exec("ROLLBACK"); throw error; }
    return tasks.map((task) => this.getTask(task.id));
  }
